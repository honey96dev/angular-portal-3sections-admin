...
@Component({
  selector: 'app-shared-media-slider-data',
  templateUrl: './shared-media-slider-data.component.html',
  styleUrls: ['./shared-media-slider-data.component.scss']
})
export class SharedMediaSliderDataComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.addUrl = sprintf("/%s/%s", this.category, routes._partials.mediaSlider.edit);
    this.editUrl = sprintf("/%s/%s", this.category, routes._partials.mediaSlider.edit);
    this.loadData();
  }
