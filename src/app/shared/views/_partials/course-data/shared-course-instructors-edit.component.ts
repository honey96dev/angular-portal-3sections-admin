...
@Component({
  selector: 'app-shared-course-instructors-edit',
  templateUrl: './shared-course-instructors-edit.component.html',
  styleUrls: ['./shared-course-instructors-edit.component.scss']
})
export class SharedCourseInstructorsEditComponent implements OnInit {
  @Input() scope: string;
...
// Your options
  ...
  ngOnInit() {
    ...
    this.config = {
      url: `${environment.apiUrl}${apis.common.upload}/course-instructors`,
    ...
    this.route.paramMap.subscribe(map => {
      ...
      this.f['targetId'].patchValue(this.targetId);
      if (this.id) {
  ...
  onSubmit() {
    ...
    const f = this.f;
    const id = f.id.value;
    const targetId = f.targetId.value;
    ...
    const data = {
      id, targetId, category, nameEn, nameAr, summaryEn, summaryAr, media, originMedia, mediaSize
    };
